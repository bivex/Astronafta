"""Generate Python parser artifacts from the vendored Astro grammar."""
...
GRAMMAR_DIR = ROOT / "resources" / "grammars" / "astro"
OUTPUT_DIR = ROOT / "src" / "swifta" / "infrastructure" / "antlr" / "generated" / "astro"
...
def _ensure_grammar_exists() -> None:
    required = (
        GRAMMAR_DIR / "AstroLexer.g4",
        GRAMMAR_DIR / "AstroParser.g4",
    )
...
def _generate_parser() -> None:
    command = [
        ...
        "-o",
        str(OUTPUT_DIR),
        str(GRAMMAR_DIR / "AstroLexer.g4"),
        str(GRAMMAR_DIR / "AstroParser.g4"),
    ]
...
def _ensure_package_files() -> None:
    ...
    if not init_file.exists():
        init_file.write_text('"""Generated Astro ANTLR parser."""\n', encoding="utf-8")

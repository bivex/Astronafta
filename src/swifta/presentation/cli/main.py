...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "parse-file":
            ...
            payload["index_path"] = str((output_dir / "index.html").resolve())
            payload["documents"] = [
                {
                    "source_location": diagram.source_location,
                    "component_count": diagram.component_count,
                    "component_names": list(diagram.component_names),
...
def _build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Parse Astro template files with ANTLR.")
    parser.add_argument("--verbose", action="store_true", help="Enable lifecycle logging.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    parse_file = subparsers.add_parser("parse-file", help="Parse one Astro file.")
    parse_file.add_argument("path", help="Path to a .astro file.")
...
def _render_directory_index(
    ...
    rows = "".join(
        (
            ...
            f"<td>{diagram.component_count}</td>"
            f"<td>{escape(', '.join(diagram.component_names) if diagram.component_names else 'No components found')}</td>"
            "</tr>"
        )
        for diagram in written_diagrams
    )
...
<html lang="en">
  <head>
    ...
    <title>Swifta Astro Index</title>
    <style>
      :root {{
        --line: #22364d;
        --page: #d9e0e7;
      ...
      * {{ box-sizing: border-box; }}
      body {{ margin: 0; padding: 24px; font-family: "Trebuchet MS", "Segoe UI", sans-serif; color: var(--text); background: linear-gradient(180deg, #e3e8ee 0%, var(--page) 100%); }}
      .window {{ max-width: 1120px; margin: 0 auto; border: 2px solid var(--line); background: var(--panel); box-shadow: var(--shadow); }}
      .titlebar {{ padding: 8px 14px; color: #ffffff; font-size: 18px; font-weight: 700; background: linear-gradient(180deg, #3394ff 0%, var(--blue) 48%, var(--blue-dark) 100%); }}
      .body {{ padding: 16px; }}
      .meta {{ margin: 0 0 14px; color: var(--muted); font-family: monospace; font-size: 12px; overflow-wrap: anywhere; }}
      ...
      th {{ color: #ffffff; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em; background: linear-gradient(180deg, var(--blue) 0%, var(--blue-dark) 100%); }}
      td:nth-child(2) {{ width: 110px; text-align: center; white-space: nowrap; }}
      a {{ color: #0f58ad; text-decoration: none; font-weight: 700; }}
      a:hover {{ text-decoration: underline; }}
    </style>
  </head>
  <body>
    <div class="window">
      <div class="titlebar">Swifta Astro Index</div>
      <div class="body">
